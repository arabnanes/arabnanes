...
export default function Search() {
  ...
  const [searchText, setSearchText] : any = React.useState(null);
  ...
  async function getSocialData(){
    ...
    const response: any = await data.search(searchText)
    return setDatas(response)
    } 
              ...
              <div className="bg-white flex flex-col rounded-md overflow-hidden h-full max-h-64vh">
                <Scrollbar className="os-host-flexbox">
                  <div className="h-full">
                    {datas.data.products.length ? (
                      <>
                        {datas.data.products.map((item  : any, index : any) => (
                          <div
