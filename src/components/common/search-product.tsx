...
const SearchProduct: React.FC<SearchProductProps> = ({ item }) => {
  ...
  return (
    <Link
      href={`${ROUTES.PRODUCT}/${item.id}/${item?.product_slug}`}
    ...
    >
      <div className="relative flex w-16 md:w-24 h-16 md:h-24 rounded-md overflow-hidden bg-gray-200 flex-shrink-0 cursor-pointer ltr:mr-4 rtl:ml-4">
        <Image
          src={
            `${siteSettings.author.websiteUrl}${item.product_thumbnail}` ?? "/assets/placeholder/search-product.svg"
          }
          width={96}
          height={96}
          loading="eager"
      ...
      <div className="flex flex-col w-full overflow-hidden">
        <h3 className="truncate text-sm text-heading mb-2">{item.product_name}</h3>
