...
const ProductSingleDetails: React.FC<Props> = ({ product }: any) => {
  ...
  const { price, basePrice } = usePrice({
    amount: product?.product.discount_price ? product?.product.discount_price : product?.product.selling_price!,
    baseAmount: product?.product.selling_price,
  });

  const variations = getVariations(product?.product.variations!);
  ...
  const combineImages = product?.multiImage;
  return (
    <div className="items-start block grid-cols-9 pb-10 lg:grid gap-x-10 xl:gap-x-14 pt-7 lg:pb-14 2xl:pb-20">
      {width < 1025 ? (
        ...
        >
          {combineImages?.length > 1 ? (
            combineImages?.map((item: any, index: any) => (
              <SwiperSlide key={`product-gallery-key-${index}`}>
                <div className="relative flex col-span-1 transition duration-150 ease-in hover:opacity-90">
                  <Image
                    ...
                    height={618}
                    src={
                      `${siteSettings.author.websiteUrl}${item?.photo_name}` ??
            ...
            <SwiperSlide key={`product-gallery-key`}>
              <div className="flex col-span-1 transition duration-150 ease-in hover:opacity-90">
                <Image
                  ...
                  height={618}
                  src={
                    `${siteSettings.author.websiteUrl}${combineImages[0].photo_name}` ??
                    '/assets/placeholder/products/product-gallery.svg'
                  }
        ...
        <div className="col-span-5 grid grid-cols-2 gap-2.5">
          {combineImages?.length > 1 ? (
            combineImages?.map((item: any, index: any) => (
              ...
              >
                <ReactImageZoom width={475} height={618} zoomWidth={500} zoomPosition="original" img={`${siteSettings.author.websiteUrl}${item?.photo_name}` ??'/assets/placeholder/products/product-gallery.svg'} />
                {/* <Image
                  width={475}
                  height={618}
                  src={
      ...
      <div className="col-span-4 pt-8 lg:pt-0">
        ...
        <div className="flex items-center py-8 space-x-4 border-b border-gray-300 rtl:space-x-reverse ltr:md:pr-32 ltr:lg:pr-12 ltr:2xl:pr-32 ltr:3xl:pr-48 rtl:md:pl-32 rtl:lg:pl-12 rtl:2xl:pl-32 rtl:3xl:pl-48">
          {isEmpty(variations) && (
            <>
              {Number(product.product.product_qty) > 0 ? (
                <Counter
                  quantity={quantity}
                  ...
                  disableDecrement={quantity === 300}
                  // disableIncrement={Number(product.product_qty) === quantity}
                />
              ) : (
