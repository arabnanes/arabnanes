...
export default function ProductPopup({ productSlug }: { productSlug: any }) {
  ...
  function addToCart() {
    ...
    const item = generateCartItem(productSlug);
  ...
  function navigateToProductPage() {
    closeModal();
    router.push(`${ROUTES.PRODUCT}/${productSlug.id}/${productSlug.product_slug}`, undefined, {
  ...
  async function getSocialData(){
    ...
    const response: any = await data.getOneProducts(productSlug.id , productSlug.product_slug)
      ...
      <div className="flex flex-col lg:flex-row w-full md:w-[650px] lg:w-[960px] mx-auto overflow-hidden">
        <div className="relative flex items-center justify-center flex-shrink-0 w-full overflow-hidden bg-gray-300 lg:w-430px aspect-[1/1.3] max-h-430px lg:max-h-full">
          <Image
            fill
            src={
              `${siteSettings.author.websiteUrl}/${productSlug.product_thumbnail}` ??
              '/assets/placeholder/products/product-thumbnail.svg'
            }
            alt={productSlug.product_name}
        ...
        <div className="flex flex-col w-full p-5 md:p-8">
          ...
          <div className="pt-2 md:pt-4">
            <div className="flex items-center justify-between mb-4 space-x-3 sm:space-x-4 rtl:space-x-reverse">
              {isEmpty(variations) && (
                <>
                  {Number(productSlug.product_qty) > 0 ? (
                    <Counter
                      quantity={quantity}
