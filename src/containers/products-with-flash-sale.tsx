...
const ProductsWithFlashSale: React.FC<Props> = ({
  ...
  async function getSocialData(){
    ...
    const response: any = await data.getTopProducts()
  ...
  return (Products) ?  (
    <div
      className={`grid grid-cols-1 gap-5 md:gap-14 xl:gap-7 xl:grid-cols-7 2xl:grid-cols-12 ${className}`}
    >
      <div className="px-4 pt-6 pb-5 border border-gray-300 rounded-lg  col-span-12 md:pt-7 lg:pt-9 xl:pt-7 2xl:pt-9 md:px-5 lg:px-7 lg:pb-7">
        <SectionHeader
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-5 xl:gap-7 xl:-mt-1.5 2xl:mt-0">
          {
            Products.data.special_deals?.map((product: any) => (
              <ProductCard
